...
const UserAccountNav = ({ user }: { user: User }) => {
  ...
  const isVendor = user?.role === 'vendor' || 'admin'
  ...
  const bgVendor = userRole === 'vendor' ? 'bg-blue-200 hover:bg-blue-100' : 'bg-yellow-200 hover:bg-yellow-100'
        ...
        {isVendor && vendor.data ? (
          <>
            <DropdownMenuItem asChild>
              <Link href='/status' className={bgVendor}>Vendor Status</Link>
            </DropdownMenuItem>
